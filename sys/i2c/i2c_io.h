 ...
 * @note: Assume this function will handle transmitting the device address and
 * setting the R/W bit before sending data.
 */
error_code_t i2cSendTo(uint8_t sAddr, uint8_t *buf, uint16_t numBytes);

/**
 * @brief Receives numBytes from the device with address sAddr and stores them
 * in buf.
 *
 ...
 * @return error_code_t An error code indicating the status of the operation.
 * @note: Assume this function will handle transmitting the device address and
 * setting the R/W bit before receiving data.
 */
