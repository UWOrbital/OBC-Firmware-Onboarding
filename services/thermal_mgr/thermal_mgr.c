...
error_code_t thermalMgrSendEvent(thermal_mgr_event_t *event) {
  ...
  if (thermalMgrQueueHandle == NULL) return ERR_CODE_UNKNOWN;

  if (xQueueSend(thermalMgrQueueHandle, event, QUEUE_WAIT_TIME) == pdPASS){
...
void osHandlerLM75BD(void) {
  ...
  interrupt.type = THERMAL_MGR_INTERRUPT;
...
static void thermalMgr(void *pvParameters) {
  ...
  while (1) {
    if((xQueueReceive(thermalMgrQueueHandle, &receivedEvent, QUEUE_WAIT_TIME)) == pdPASS){
      float curr_temp;
      error_code_t status;
      status = readTempLM75BD(data.devAddr, &curr_temp);

      if(status == ERR_CODE_SUCCESS){
        if(receivedEvent.type == THERMAL_MGR_EVENT_MEASURE_TEMP_CMD){
          addTemperatureTelemetry(curr_temp);
        }
        else if(receivedEvent.type == THERMAL_MGR_INTERRUPT){
