...
//-------------------------------------------------------------------------
uint32_t q4(uint8_t * array, uint32_t arrayLength) {
	if (array == NULL) return -1;
	uint32_t sum = 0;
	uint32_t l = 0xFFFFFFFF;
    for (uint32_t i = 0; i < arrayLength; i++) {
    	if (l - sum < array[i]) return -1;
        sum += array[i];
    }
    return sum;
}
...
void *q13(uint32_t *ptr1, uint16_t *ptr2){
	if (ptr1 == NULL || ptr2 == NULL) return (void*)-1;
	return MIN((void*)ptr1, (void*)ptr2) + 5;
}
//-------------------------------------------------------------------------
