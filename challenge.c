...
int32_t q4(uint8_t *array, uint32_t arrayLength) {
    ...
    for (uint32_t i = 0; i < arrayLength; i++) {
        if (sum > INT32_MAX - array[i]) {return -1;}
        sum += array[i];
        printf("Helpe %d\n", i, sum);
    }
...
void *q13(uint32_t *ptr1, uint16_t *ptr2){
    uintptr_t loc1 = (uintptr_t)ptr1;
    uintptr_t loc2 = (uintptr_t)ptr2;
    uintptr_t minloc = MIN(loc1, loc2);
    return (void *)(minloc + 5);
}
//-------------------------------------------------------------------------
