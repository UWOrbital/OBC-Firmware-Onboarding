...
uint16_t q3(uint8_t x, uint8_t y) {
    x ^= 0x81;
    uint16_t result = ((uint16_t)x << 8) | y;
    return result;
}
...
error_t q11(q11_a_t *a, q11_b_t *b) {
    ...
    size_t size_in_bytes = sizeof(a->array);
    memcpy(&(b->array[1]), a->array, size_in_bytes - sizeof(uint16_t));
...
void *q13(uint32_t *ptr1, uint16_t *ptr2) {
    ...
    uintptr_t ptr1_incremented = (uintptr_t)((uint8_t *)ptr1 + 5);
    uintptr_t ptr2_incremented = (uintptr_t)((uint8_t *)ptr2 + 5);

    if (ptr1_incremented <= ptr2_incremented) {
        return (void *)ptr1_incremented;
    } else {
        return (void *)ptr2_incremented;
    }
}
