...
// - uint16_t b
//-------------------------------------------------------------------------
typedef union {uint32_t a; uint16_t b;} q5_t;
...
// - uint16_t y
//-------------------------------------------------------------------------
typedef struct {uint32_t x; uint16_t y;} q6_t;
...
// - FAIL = 1
//-------------------------------------------------------------------------
typedef enum { SUCCESS = 0, FAIL = 1} error_t;
...
void *q13(uint32_t *ptr1, uint16_t *ptr2){
    ...
    uintptr_t min = (addr1 < addr2) ? addr1 : addr2;
