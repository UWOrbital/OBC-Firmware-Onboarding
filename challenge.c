...
uint16_t q3(uint8_t x, uint8_t y) {
    x^=0x80;
    x^=0x01;
    uint16_t result=((uint16_t)x << 8) | (uint16_t)y;
    return result;
}
...
error_t q11(q11_a_t *a, q11_b_t *b){
    ...
    memmove(b->array+1,a->array,sizeof(a->array));
    return SUCCESS;
}
...
void *q13(uint32_t *ptr1, uint16_t *ptr2){
    ...
    uintptr_t addr1=(uintptr_t)ptr1+5;
    uintptr_t addr2=(uintptr_t)ptr2+5;
    return (void*)(addr1 < addr2?addr1:addr2);
}
//-------------------------------------------------------------------------
