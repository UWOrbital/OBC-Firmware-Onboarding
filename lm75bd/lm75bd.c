...
error_code_t readTempLM75BD(uint8_t devAddr, float *temp)
{
  ...
  error_code_t result = i2cSendTo(devAddr, &sendBuf, sizeof(sendBuf));
  if (result != ERR_CODE_SUCCESS)
  {
    return result;
  }

  result = i2cReceiveFrom(devAddr, recvBuf, sizeof(recvBuf));
  if (result != ERR_CODE_SUCCESS)
  {
    return result;
  }

  // Combine the received bytes and convert to Celsius
  int16_t rawTemp = (recvBuf[0] << 8) | recvBuf[1];
