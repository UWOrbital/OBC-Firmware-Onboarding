...
error_code_t readTempLM75BD(uint8_t devAddr, float *temp)
{
  ...
  /* write pointer register =0x00, read 2 bytes from sensor, convert 11-bit 2's complement temp format into celcius */
  ...
  err = i2cSendTo(devAddr, &pointer, 1);
  if (err != ERR_CODE_SUCCESS)
    return err;

  // Read 2 bytes
  uint8_t buf[2] = {0};
  err = i2cReceiveFrom(devAddr, buf, 2);
  if (err != ERR_CODE_SUCCESS)
    return err;

  // combine msb and lsb into 11-bit 2's complement
  int16_t combined_raw = ((int16_t)buf[0] << 8) | buf[1];

  int16_t rawTemp = combined_raw >> 5; // shift to get rid of unused bits

  *temp = rawTemp * 0.125f; // each bit represents 0.125 degrees celsius
