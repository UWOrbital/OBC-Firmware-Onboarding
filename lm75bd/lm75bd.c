...
error_code_t readTempLM75BD(uint8_t devAddr, float *temp) {
  ...
  */
  ...
  uint8_t WriteBuff = LM75BD_PTR_BYTE;
  ...
  if((preTemp & (1 << 15)) == 1){//shift 1 left 15 bits to test d10 if it's a 1
    //negative temp, take twos compliment
    ~preTemp;//invert bits
    preTemp+1;//add 1
  }

  float convTemp = preTemp*0.125;//need to retain numerical accuracy here. Temp is in ° celcius
